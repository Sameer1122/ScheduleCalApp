...
const CreateEventDetail = ({ detailsForm, eventType, eventDetails, setEventDetails, slug, locationOptions }: any) => {
  const router = useRouter();
  const { t } = useLocale();
  const [showLocationModal, setShowLocationModal] = React.useState(false);
  const [selectedLocation, setSelectedLocation] = React.useState<OptionTypeBase | undefined>(undefined);
  const [locationType, setLocationType] = React.useState('');
  const [locationAddress, setLocationAddress] = React.useState('');
  const [locationLink, setLocationLink] = React.useState('');

  const openLocationModal = (type: LocationType) => {
    setSelectedLocation(locationOptions.find((option: any) => option.value === type));
    setShowLocationModal(true);
  };

  const handleModalOK = () => {
    detailsForm.setFieldsValue({ ...detailsForm.getFieldsValue(), eventLocationAddress: locationAddress || "", eventLocationLink: locationLink || "" });
  ...
  const addLocation = (newLocationType: LocationType, details = {}) => {
    const existingIdx = detailsForm.getFieldValue("eventLocations").findIndex((loc: any) => newLocationType === loc.type);
    if (existingIdx !== -1) {
      const copy = detailsForm.getFieldValue("eventLocations");
      copy[existingIdx] = {
        ...detailsForm.getFieldValue("eventLocations")[existingIdx],
        ...details,
      };
      const fields = { ...detailsForm.getFieldsValue(), eventLocations: copy }
      detailsForm.setFieldsValue(fields);
    } else {
      const fields = { ...detailsForm.getFieldsValue(), eventLocations: detailsForm.getFieldValue("eventLocations").concat({ type: newLocationType, ...details }) }
      detailsForm.setFieldsValue(fields);
    }
  };

  const removeLocation = (selectedLocation: typeof eventType.locations[number]) => {
    const fields = { ...detailsForm.getFieldsValue(), eventLocations: detailsForm.getFieldValue("eventLocations").filter((location: any) => location.type !== selectedLocation.type) }
    detailsForm.setFieldsValue(fields);
  };
  ...
  const Locations = () => {
    return (
      <div className="w-full">
        {detailsForm.getFieldValue("eventLocations").length === 0 && (
          <Select
            style={{ width: "100%" }}
            allowClear
            ...
            onChange={(value: any) => {
              if (value) {
                const newLocationType: LocationType = value;
        ...
        {detailsForm.getFieldValue("eventLocations").length > 0 && (
          <ul>
            {detailsForm.getFieldValue("eventLocations").map((location: any) => (
              <li
                key={location.type}
            ...
            {detailsForm.getFieldValue("eventLocations").length > 0 &&
              detailsForm.getFieldValue("eventLocations") !== locationOptions.length && (
                <li>
                  <button
                    type="button"
    ...
    <>
      ...
      >
        <Space direction="vertical" style={{ width: "100%" }}>
          <TopText>Event Detail</TopText>
          <DescriptionText>
            Name your event and provide a description for your clients.
          </DescriptionText>
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <Form.Item
              name="eventDesc"
            >
              <Input placeholder="Write a summary and any details your invitee should know about the event." />
            </Form.Item>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              <Input placeholder="event-name" addonBefore={`${process.env.NEXT_PUBLIC_WEBSITE_URL}/${slug}/`} />
            </Form.Item>
          </div>
        </Space>
      </Form>
...
const timeOption = [
  ...
  { value: 60, text: "60 minutes (Default)" },
  { value: 90, text: "90 minutes" },
  { value: 120, text: "120 minutes" },
  { value: 150, text: "150 minutes" },
  { value: 180, text: "180 minutes" },
  { value: 210, text: "210 minutes" },
];
