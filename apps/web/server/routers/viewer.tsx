...
const loggedInViewerRouter = createProtectedRouter()
  ...
  .query("bookings", {
    input: z.object({
      status: z.enum(["upcoming", "past", "cancelled", "reports"]),
    ...
    async resolve({ ctx, input }) {
      ...
      if(bookingListingByStatus == 'reports') {
        return {
          bookings: [],
          nextCursor: null,
        }
  ...
  .query("bookingReports", {
    ...
    async resolve({ ctx, input }) {
      const { prisma, user } = ctx;
      const bookingListingFilters:any = {};
      if(input.startTime) {
        bookingListingFilters.startTime = { gte: input.startTime }
      }
      if(input.endTime) {
        bookingListingFilters.endTime = { gte: input.endTime }
      }
      
      const appointments = await prisma.booking.aggregate({
        _count: {
          id: true,
        },
        where: {
      ...
      const cancelleds = await prisma.booking.aggregate({
        _count: {
          id: true,
        },
        where: {
