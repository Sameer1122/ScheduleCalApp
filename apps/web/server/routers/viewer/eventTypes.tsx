...
function handleNewCustomInputs(customInputs: EventTypeCustomInput[]) {
  const cInputsToCreate = customInputs
    .filter((input) => input.id < 0)
    .map((input) => ({
      type: input.type,
...
const EventTypeCreateFullInput = _EventTypeModel
  .extend({
...
export const eventTypesRouter = createProtectedRouter()
  ...
  .mutation("full_create", {
    input: EventTypeCreateFullInput,
    async resolve({ ctx, input }) {
      const { schedule, periodType, customInputs, ...rest } = input;
      const userId = ctx.user.id;
      ...
      if (customInputs) {
        data.customInputs = handleNewCustomInputs(customInputs);
      }
